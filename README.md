...
| `vibecheck` | Аудитор проекта: ищет утечки ключей, дыры в безопасности и мёртвый код по всему репозиторию. Детерминированные python-скрипты без зависимостей плюс проход модели поверх. Отчёт устроен так, чтобы починить самому: уровень срочности → что нашли → почему это опасно простым языком → как исправить. | [skills/vibecheck](skills/vibecheck/) |
| `humanvoice` **v2.5** | Редактор живого русского: находит и убирает следы нейросети (канцелярит, вода, машинные шаблоны, типографика) и возвращает тексту человеческий голос. Два режима — диагностика (пахнет ли ИИ и чем) и правка. Смотрит на текст как на 4 слоя следов, не выдумывает факты, не переусердствует. | [skills/humanvoice](skills/humanvoice/) |
